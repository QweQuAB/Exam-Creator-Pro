  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid gap-8 lg:grid-cols-3">
        {/* Exams List with Section A / B tabs */}
          ...
          <Tabs defaultValue="section-a" className="space-y-4">
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="section-a" className="gap-2">
                <FileText className="h-4 w-4" /> Section A — MCQ
              </TabsTrigger>
              <TabsTrigger value="section-b" className="gap-2">
                <PenLine className="h-4 w-4" /> Section B — Essay
              </TabsTrigger>
            </TabsList>

            {/* Section A — shows all exams for their MCQ content */}
            <TabsContent value="section-a">
              <div className="grid gap-4 sm:grid-cols-2">
                {isLoadingExams ? (
                  ...
                  examList.map((exam) => (
                    <ExamCard key={exam.id} exam={exam} section="a" />
                  ))
                )}
              </div>
            </TabsContent>

            {/* Section B — links into the essay tab of each exam */}
            <TabsContent value="section-b">
              <div className="grid gap-4 sm:grid-cols-2">
                {isLoadingExams ? (
                  ...
                  examList.map((exam) => (
                    <ExamCard key={exam.id} exam={exam} section="b" />
                  ))
                )}
              </div>
            </TabsContent>
...
function ExamCard({ exam, section }: { exam: any; section: "a" | "b" }) {
  const isEssay = section === "b";
  return (
      ...
      <CardContent className="flex-1 pb-3 text-sm text-muted-foreground">
        <div className="flex justify-between items-center">
          <span className="flex items-center gap-1">
            {isEssay ? <PenLine className="w-3 h-3" /> : <FileText className="w-3 h-3" />}
